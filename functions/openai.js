...
const configs = path.join(__dirname, '../configs/configs.json')
const contents = fs.readFileSync(configs);

const configuration = new Configuration({
  apiKey: JSON.parse(contents).apiKey,
});
const openai = new OpenAIApi(configuration);
const accessToken = JSON.parse(contents).accessToken;
const uri = 'https://graph.facebook.com/v13.0/me/messages';
const sendMessage = require("./request.js")

module.exports = async (messageText, senderId) => {
  //openai
  try {
    if (messageText.includes('/help')) {
      const messageData = {
        ...
        message: {
          text: "🤖Commands: \n\n ✅/ai 👉 classic GPT AI \n\n ✅/dan 👉 AI with no limitations \n\n ✅/dev 👉 AI [Developer Mode]\n\n ✅/math 👉 AI [MATH PRO]"
        }
      };
      sendMessage(uri, accessToken, messageData)
        .then(response => {
          console.log(1);
        })
        .catch(error => {
    ...
    if (messageText.includes('/ai')) {
      ...
      if (messageText.length == 0) {
        const messageData = {
          recipient: { id: senderId },
          message: { text: "⚠️Please enter a message!⚠️" }
        };
        sendMessage(uri, accessToken, messageData)
          .then(response => {
            console.log(1);
    ...
    //dan
    if (messageText.includes('/dan')) {
      messageText = messageText.replace('/dan', '')
      if (messageText.length == 0) {
        const messageData = {
          recipient: { id: senderId },
          message: { text: "⚠️Please enter a message!⚠️" }
    ...
    //math
    if (messageText.includes('/math')) {
      messagesText = messageText.replace('/math', '')
      if (messageText.length == 0) {
        const messageData = {
          recipient: { id: senderId },
          message: { text: "⚠️Please enter a message!⚠️" }
    ...
    //evil
    if (messageText.includes('/evil')) {
      messagesText = messageText.replace('/evil', '')
      if (messageText.length == 0) {
        const messageData = {
          recipient: { id: senderId },
          message: { text: "⚠️Please enter a message!⚠️" }
    ...
    //dev
    if (messageText.includes('/dev')) {
      messagesText = messageText.replace('/dev', '')
      if (messageText.length == 0) {
        const messageData = {
          recipient: { id: senderId },
          message: { text: "⚠️Please enter a message!⚠️" }
